...
module.exports = function (data) {
	...
	.then(() => makeTruthyPromise(data.mergeBase)
			.then(mergeBase => getLastKnownCoverageOfCommit(data.project, mergeBase, data.projectName))
		.catch(() => makeTruthyPromise(data.mergeRequestIID)
			.then(mergeRequestIID => getMergeBaseFromMR(data.project, mergeRequestIID, data.token, data.tokenType))
			.then(mergeBase => getLastKnownCoverageOfCommit(data.project, mergeBase, data.projectName)))
		.catch(() => makeTruthyPromise(data.targetBranch)
			.catch(() => gitlabHelper.getDefaultBranch(data.project, data.token, data.tokenType))
			.catch(q.resolve('master'))
			.then(branchName => getLastKnownCoverageOfBranch(data.project, branchName, data.projectName)))
	)
...
var getMergeBaseFromMR = function (projectId, mergeRequestIID, token, tokenType) {
	return gitlabHelper.getMergeRequest(projectId, mergeRequestIID, token, tokenType)
		.then(mr => mr.diff_refs.base_sha);
};
