...
module.exports = {
	...
	getDefaultBranch: function (projectId, token, tokenType) {
		...
		return this.getProject(projectId, token, tokenType)

		.then(function (project) {
			return q(project.default_branch);
		});
	},
	...
	getProject: function (projectId, token, tokenType) {
		var deferred = q.defer();
	...
	getMergeRequest: function (projectId, mergeRequestIID, token, tokenType) {
		var deferred = q.defer();

		restler.get(gitlab.api + 'projects/' + projectId + '/merge_requests/' + mergeRequestIID + '?' + tokenType + '=' + token)
